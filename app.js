...
async function lookup(value){
  ...
  // not found anywhere
  currentMode = 'UNKNOWN';
  currentRecord = { upc: value, article:'', deskripsi:'' };
...
async function showViewModal(){
  ...
  if(arr.length === 0){
    ...
    // build table
    let html = '<table><thead><tr><th>UPC</th><th>Article</th><th>Deskripsi</th><th>Qty</th><th>Keterangan</th><th>Waktu</th><th>Foto</th><th>Hapus</th></tr></thead><tbody>';
...
async function exportOpnameCsv(){
  ...
  const header = ['upc','article','deskripsi','qty','keterangan','timestamp','DiInputOleh'];
  ...
  try {
    ...
    for (const src of csvSources) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        buffer.push({ upc, article, deskripsi });
        existingUPC.add(upc);
        sukses++;
